...
drop table if exists OBBLIGATIONS;
...
drop table if exists PASSED_EXMES;

drop table if exists POPULATED_PLACES;

drop table if exists PROFESSIONS;

drop table if exists PROGRAM_EVIDENCE;
...
drop table if exists STUDENT_STUDY_DATA;

drop table if exists STUDENT___PERSONAL_DATA;
...
(
   ...
   VU_IDENTIFIKATOR     int not null,
   primary key (HIG_TIP_UST, TIP_UST, ACT_VU_IDENTIFIKATOR, AO_REDNI_BROJ, VU_IDENTIFIKATOR)
);
...
/* Table: OBBLIGATIONS                                          */
/*==============================================================*/
create table OBBLIGATIONS
(
   HIG_TIP_UST          char(2) not null,
   VU_IDENTIFIKATOR     int not null,
   PG_IDENT             char(2) not null,
   STU_EVIDENCE_NUMBER  numeric(4,0) not null,
   STU_EVIDENCE_YER     numeric(4,0) not null,
   OBL_ID               smallint not null,
   NP_PREDMET           varchar(6) not null,
   NP_VERZIJA           numeric(2,0) not null,
   OBL_TAKEN_DATE       date,
   ZAP_REDNI_BROJ       numeric(6,0) not null,
   primary key (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT, STU_EVIDENCE_NUMBER, STU_EVIDENCE_YER, OBL_ID)
);
...
(
   ...
   PASS_POINTS          decimal(5,2) not null,
   PASS_KLASIFICATION   char(2) not null,
   primary key (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT, STU_EVIDENCE_NUMBER, STU_EVIDENCE_YER)
);
...
(
   ...
   VU_IDENTIFIKATOR     int not null,
   PG_IDENT             char(2) not null,
   PG_NAME              varchar(120) not null,
   TIPP_TIP             char(1) not null,
   SP_EVIDENCIONI_BROJ  int not null,
   SP_VERZIJA           numeric(2,0) not null,
   primary key (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT)
);
...
/*==============================================================*/
/* Table: STUDENT_STUDY_DATA                                    */
/*==============================================================*/
create table STUDENT_STUDY_DATA
(
   HIG_TIP_UST          char(2) not null,
   VU_IDENTIFIKATOR     int not null,
   PG_IDENT             char(2) not null,
   ...
   STU_TAKEN            numeric(3,0) not null default 0,
   STU_VERIFIED         numeric(3,0) not null default 0,
   primary key (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT, STU_EVIDENCE_NUMBER, STU_EVIDENCE_YER)
);

/*==============================================================*/
/* Table: STUDENT___PERSONAL_DATA                               */
/*==============================================================*/
create table STUDENT___PERSONAL_DATA
(
   TIP_UST              char(2) not null,
   VU_IDENTIFIKATOR     int not null,
   STU_ID               bigint not null,
   ...
   primary key (TIP_UST, VU_IDENTIFIKATOR, STU_ID)
);
...
alter table INSTITUTION_STRUCTURE add constraint FK_WHO_IS_IN_ORGANIZATION foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR)
      references HIGH_EDUCATION_INSTITUTION (TIP_UST, VU_IDENTIFIKATOR) on delete restrict on update restrict;
...
alter table OBBLIGATIONS add constraint FK_ATTENDED_COURSES foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, ZAP_REDNI_BROJ)
      references EMPLOYEES (TIP_UST, VU_IDENTIFIKATOR, ZAP_REDNI_BROJ) on delete restrict on update restrict;

alter table OBBLIGATIONS add constraint FK_HAS_TO_FULLFILE foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT, STU_EVIDENCE_NUMBER, STU_EVIDENCE_YER)
      references STUDENT_STUDY_DATA (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT, STU_EVIDENCE_NUMBER, STU_EVIDENCE_YER) on delete restrict on update restrict;

alter table OBBLIGATIONS add constraint FK_IN_OBLIGATION foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, NP_PREDMET, NP_VERZIJA)
      references COURSE (TIP_UST, VU_IDENTIFIKATOR, NP_PREDMET, NP_VERZIJA) on delete restrict on update restrict;
...
alter table PASSED_EXMES add constraint FK_OTORISED foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, ZAP_REDNI_BROJ)
      references EMPLOYEES (TIP_UST, VU_IDENTIFIKATOR, ZAP_REDNI_BROJ) on delete restrict on update restrict;

alter table PASSED_EXMES add constraint FK_PASSED foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT, STU_EVIDENCE_NUMBER, STU_EVIDENCE_YER)
      references STUDENT_STUDY_DATA (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT, STU_EVIDENCE_NUMBER, STU_EVIDENCE_YER) on delete restrict on update restrict;

alter table PASSED_EXMES add constraint FK_PASSED_BY foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, NP_PREDMET, NP_VERZIJA)
      references COURSE (TIP_UST, VU_IDENTIFIKATOR, NP_PREDMET, NP_VERZIJA) on delete restrict on update restrict;

alter table POPULATED_PLACES add constraint FK_POPULATED_PLACES foreign key (DR_IDENTIFIKATOR)
      references STATE (DR_IDENTIFIKATOR) on delete restrict on update restrict;

alter table PROGRAM_EVIDENCE add constraint FK_EVIDENCE foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR)
      references HIGH_EDUCATION_INSTITUTION (TIP_UST, VU_IDENTIFIKATOR) on delete restrict on update restrict;

alter table PROGRAM_EVIDENCE add constraint FK_IN_THE_EVIDENCE foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, TIPP_TIP, SP_EVIDENCIONI_BROJ, SP_VERZIJA)
      references REGISTERED_CURRICULUMS (TIP_UST, VU_IDENTIFIKATOR, TIPP_TIP, SP_EVIDENCIONI_BROJ, SP_VERZIJA) on delete restrict on update restrict;
...
alter table STATE add constraint FK_CAPITAL foreign key (DR_IDENTIFIKATOR, NM_IDENTIFIKATOR)
      references POPULATED_PLACES (DR_IDENTIFIKATOR, NM_IDENTIFIKATOR) on delete restrict on update restrict;
...
alter table STUDENT_STUDY_DATA add constraint FK_EVIDENCE_IDENT foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT)
      references PROGRAM_EVIDENCE (HIG_TIP_UST, VU_IDENTIFIKATOR, PG_IDENT) on delete restrict on update restrict;

alter table STUDENT_STUDY_DATA add constraint FK_IN_EVIDENCE foreign key (HIG_TIP_UST, VU_IDENTIFIKATOR, STU_ID)
      references STUDENT___PERSONAL_DATA (TIP_UST, VU_IDENTIFIKATOR, STU_ID) on delete restrict on update restrict;

alter table STUDENT___PERSONAL_DATA add constraint FK_STUDY_AT foreign key (TIP_UST, VU_IDENTIFIKATOR)
      references HIGH_EDUCATION_INSTITUTION (TIP_UST, VU_IDENTIFIKATOR) on delete restrict on update restrict;
